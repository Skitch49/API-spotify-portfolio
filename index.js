...
//Steam params
const urlSteam = "https://api.steampowered.com/";
const steamApiKey = process.env.API_KEY_STEAM;
...
app.get("/api/spotify/top", ensureValidToken, async (req, res) => {
  ...
  try {
    ...
    if (error.response && error.response.status === 401) {
      ...
      console.error(
        `Erreur lors de la récupération des meilleurs ${type}:`,
        error
      );
...
app.get("/api/spotify/current-track", ensureValidToken, async (req, res) => {
  try {
    ...
    if (error.response && error.response.status === 401) {
      ...
      console.error(
        "Erreur lors de la récupération du morceau en cours de lecture:",
        error
      );
      res.status(500).send("Erreur serveur");
    }
...
// Steam
app.use("/api/steam/user", async (req, res) => {
  try {
    const response = await axios.get(
      `${urlSteam}ISteamUser/GetPlayerSummaries/v0002/?key=${steamApiKey}&steamids=${steamID}`
    );
    res.json(response.data);
  } catch (error) {
    console.error(`Erreur récupération données steam du joueur: ${error}`);
    res.status(500).send(`Erreur récupération données steam du joueur: ${error}`);
  }
});

app.use("/api/steam/game", async (req, res) => {
  try {
    const response = await axios.get(
      `${urlSteam}IPlayerService/GetOwnedGames/v0001/?key=${steamApiKey}&steamid=${steamID}&format=json`
    );

    res.status(200).send(response.data);
  } catch (error) {
    console.error(`Erreur récupération des jeux steam du joueur: ${error}`);
    res.status(500).send(`Erreur récupération des jeux steam du joueur: ${error}`);
  }
});
